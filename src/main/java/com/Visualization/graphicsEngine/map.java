package com.Visualization.graphicsEngine;
...
/**
 * 要用Graphics2D类的新方法画一个图形。
 * 先在重画方法paintComponent()或paint()中，把参数对象Graphics g 强制转换成Graphics2D对象 g2；
 * 然后，用上述图形类提供的静态内部类构造方法Double()或Float()创建该图形的对象 ------ Line2D - 线段类；RoundRectangle2D - 圆角矩形类；Ellipse2D - 椭圆类；Arc2D - 圆弧类；QuadCurve2D - 二次曲线类；CubicCurve2D - 三次曲线类。
 * 最后，以图形对象为参数调用Graphics2D对象的draw()方法绘制这个图形。
 *
 * http://blog.csdn.net/xietansheng/article/details/55669157
 */
public class map {
    ...
    public static class MyPanel extends JPanel {
        ...
        private void drawCurves(Graphics g) {
            ...
            Points points = CurveManager.getPointsByCurveNum(0);
            int nPoints = points.points.size();
            int[] xPoints = new int[nPoints];
            int[] yPoints = new int[nPoints];
            for (int i = 0; i < points.points.size(); i++) {
                //仅作为测试使用，实际上坐标点应该设计一个比例
                xPoints[i] = (int) points.points.get(i).getX() * 5;
                yPoints[i] = (int) points.points.get(i).getY() * 10;
            }
